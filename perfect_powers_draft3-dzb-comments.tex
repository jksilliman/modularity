...
\david{This looks great. My main comment is that its probably better to write Theorem 3 than Theorem 3 (Theorem 4.1), and later say ``now we prove theorem 3''.}


\david{Also, your ``rad'' macro is giving the wrong font on its argument.}
...
The sequence $u_0$ \david{do you mean ``$u$', or ``$u_0,u_1,\ldots$''?} is termed a Lucas sequence, and will be our main focus.  The $n$th terms of these sequences are given by 
...
We first deal with the case of $p=2$; we will derive our contradiction from the relation \eqref{gen_diophan} \david{(3)}, which in this case reduces to
...
\[ x^n +y^n = z^2 \]
\david{display is missing period.}
But there are no nontrivial solutions to the $(n,n,2)$ Diophantine equation for $n \geq 4$  \cite{darmon97} .  We easily check that in our examples, there are no solutions for $n=3$. \david{easier: you can just check that $u_2,u_3$ are not squares or cubes.}  In the case $n$ is even, write $n=2^rk$ for $k$ odd.  Then we can write
\begin{align*}
...
\david{If $u$ and $v$ are only relatively prime away from a finite set of primes, then I think a variant of this argument works for any given sequence.}
...
Our proof relies heavily on the following two deep theorems . \david{extra space}
...
for $\rho_{f,p}$ the 2-dimensional $p$-adic  Galois representation of \david{``attached to'' instead of ``of''?} $f$ and $\bar{\rho_{f,p}}$ the reduction of $\rho_{f,p}$ mod $p$.
\end{thm}

\begin{thm}[Level Lowering \cite{ribet91}]\label{levellow}
Let $f$ be a newform of level $\ell N$ with absolutely irreducible 2-dimensional mod $p$ Galois representation $\bar{\rho_{f,p}}$ unramified at $\ell$ if $\ell \neq p$ and finite flat at $\ell = p$.  Then there exists a weight 2 newform $g$ of level $N$ such that
\[ \bar{\rho_{f,p}} \simeq \bar{\rho_{g,p}}. \]
\end{thm}


In general, to a hypothetical solution of a Diophantine equation with exponent $p$ we associate a Frey elliptic curve with coefficients depending on the solution.  To these Frey curves we associate the mod $p$ Galois representation \david{Pedantically -- should you define $\rho_{E,p}$ earlier?}
\[ \rho_{E,p}: G_{\Q} \rightarrow \GL_2(\F_p) \]
\david{Functions should be texed as $f\colon X \to Y$, not $f : X \to Y$ } corresponding to the $p$-torsion number field $\Q(E[p])$ \david{Better to say ``action of ... on the p-torsion''}.  Note that as the Galois action respects the Weierstrass \david{chante Weierstrass to elliptic} curve group law, $\im(\rho_{E,p}) \subseteq \GL_2(\F_p)$. These Frey curves will have minimal discriminant of the form $\Delta_E = C \cdot D^p$ for $C$ depending only upon the Diophantine equation, and $D$ depending only upon the hypothetical solution.  The goal is to either to use level lowering to show that the Frey curve is not modular, and thus cannot exist, or to derive local information about a solution that could exist. 
...
This is a solution to the $(p,p,2)$-with-coefficients \david{twisted is the standard word} generalized Fermat equation
...
As a convention let $k = \ord_2(b^2+4c)$, \david{first part is redundant}  $b^2+4c = 2^kD$, and $w_n = \pm v_n$ as necessary in the situation (if no congruence conditions are stated then $w_n = v_n$).  Without loss of generality we may assume that we are in one of the following situations:
...
In order to derive a contradiction or nontrivial local data, we first prove that the mod $p$ representation is unramified at any odd prime \david{missing words here?} depending \emph{only} on the solution, and that it absolutely irreducible at $p \geq 7$, and \david{in} some cases $p=5$.  In this way, we can apply Theorem \ref{levellow} and level-lower to a level independent of the solution.  


\begin{lem}\label{unram}
The mod $p$ Galois representation $\rho_{E,p}$ associated to one of the above Frey curves is unramified at any odd prime $\ell$ not dividing $c \cdot (b^2+4c)$.  Further it is \david{finite} flat at $p$ if $p \notdiv 2c\cdot (b^2+4c)$.
\end{lem}
\begin{proof}
By the N\'{e}ron-Ogg-Shafarevich criterion, the mod $p$ Galois representation is unramified outside $pN$.  By the theory of Tate curves, \david{you really want $\Q^{\text{ur}}_\ell$ here.} $E(\bar{\Q}_\ell) \simeq \bar{\Q}_\ell^\ast / q^{\Z}$ with $E[p](\bar{\Q}_\ell) = \langle \zeta_p, q^{1/p} \rangle$, so in particular the ramification comes from primes $\ell$ ramified in $\Q(\zeta_p)$ and those such that $p \notdiv \upnu_\ell(q) = \upnu_\ell(\Delta)$.  If $\ell | y$, then as $p \mid \upnu_\ell(\Delta)$, $\rho_{E,p}$ is unramified at $\ell$.  Further as $\Q(\zeta_p) \subset \Q(E[p])$, $\rho_{E,p}$ will be ramified at $p$; however, if $p \mid N$, but $p \notdiv 2c \cdot (b^2+4c)$, then $p \mid y$, and the above argument shows this does not contribute to the ramification \david{the last sentence needs clarified.}.
\end{proof}

\begin{lem}\label{absirr}
Let $E$ be an elliptic curve with a rational $2$-torsion point.   
\begin{enumerate}[(i)]
\item If there exists a prime of multiplicative reduction for $E$, then $\rho_{E,p}$ is absolutely irreducible for $p \geq 7$\david{.}
\item Further if $\Delta_E$ is a square, then $\rho_{E,5}$ is absolutely irreducible.
\end{enumerate}
\end{lem}
\begin{proof}
An irreducible \david{reducible} mod $p$ representation implies that $E$ has a $\Q$-rational $p$ isogeny.  And as $\rho_{E,p}$ is odd \david{colon} ($\det: G_\Q \rightarrow \F_p^*$ is the cyclotomic character $\chi_p$ and $\chi_p(c)=-1$ for $c$ complex conjugation), irreducible implies absolutely irreducible.  
