...
\begin{equation}\label{gen_diophan_unred}(\alpha - \beta)^2u_n^2 = v_n^2 - 4(\alpha\beta)^n. \end{equation}, i.e.
...
Further, for sequences with $c = 1$, $d = b^2 + 4c$ prime, we can  easily deal wtih $p \leq 17$. For a solution $(n,y,p)$ to \eqref{gen_diophan}, with can eliminate $n$ even via an argument of Robbins **CITE** (although it is written for the fibonacci sequence, it applies to all Lucas sequences with $c = 1$, $d = b^2 + 4c$ prime).  When $n$ is odd, we imitate \cite{siksek06}, converting solutions $(n,y,p)$ to \eqref{gen_diophan} into solutions $(X,Y) \in \Z^2$ of the Thue equation \begin{equation}
\pm 1 = F(X,Y) = A \sum_{k=0}^{\lfloor \frac{p}{2} \rfloor} (-4)^{\frac{p-2k-1}{2}} {p\choose 2k} X^{2k}Y^{p-2k} + \frac{B}{2}\sum_{k=0}^{\lfloor \frac{p}{2} \rfloor} (-4)^{\frac{p-2k-1}{2}} {p\choose 2k+1} X^{2k+1}Y^{p-2k-1},
\end{equation} where $d = A^2+B^2$, $B$ even, and $y^2 = 4X^2 + Y^2$. We then obtain an upper bound $\max\{|X|, |Y|\} < B$ using the work of Bugeaud and Gyory on Thue equations \cite{bugeaud96}. Since $y^p = [ \frac{\alpha^n}{\sqrt{d}} ]$, we get a bound on n: \begin{equation}
n < \frac{\log(\sqrt{5^p d}B^p + \sqrt{d})}{\log(\alpha)}.
\end{equation}
