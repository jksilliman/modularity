...
Solutions of the form $0=0^p$ or $1 = 1^p$ are called trivial, as they are perfect $p$th powers for any $p$.   We prove the following theorem on perfect powers in a class of Lucas sequences where $b^2+4c = 1$.
...
be the Dedekind $\psi$ function.  Consider a solution (n,y,p) to \eqref{the_eqn}, with $n > 6$ and $b^2+4c >0$. For $b,c$ relatively prime, let $N = 2^8 \cdot \rad'(c)\rad'(b^2+4c)$; otherwise let $N = 2^8 \cdot \rad'(c)^2\rad'(b^2+4c)$, where $\rad'$ denotes the product over odd primes. Then 
...
\end{ithm}
Using this theorem, we prove conditional results for several sequences in Section \ref{examples}.  For $b$ even and $c = -1$, our result improves upon one published by Bennett in \cite{bennett05}.
...
\begin{equation}\label{gen_diophan_unred}(\alpha - \beta)^2u_n^2 = v_n^2 - 4(\alpha\beta)^n, \end{equation} which is equivalent to
\begin{equation}\label{gen_diophan}(b^2+4c)u_n^2 = v_n^2 - 4(-c)^n. \end{equation}
...
\begin{rem}
We believe the examples in Theorem \ref{explicit_eg_thm} are the only sequences with $b^2+4c = 1$ that map to levels $N$ such that $\dim S_2(\Gamma_0(N)) = 0$. Finiteness is clear as we may transform the problem into a question of integral points on an elliptic curve.  We have computationally checked that these are all solutions with $|b|,|c| \leq 10^{50}$.
\end{rem}
...
\end{proof}
 ...
 Let $E$ be a Frey curve, such as in \ref{freycurves}.  Then 
...
\[N_f =  2^\alpha {\rad}'(c(b^2+4c))\leq 2^8  {\rad}'(c(b^2+4c)) = N.\]
 ...
 If $f$ is rational, then we apply Theorem \ref{bound_ell} to conclude that $p \leq \max\left\{ 17, 3 \log|\alpha| \cdot (N+1) \right\}$. 
...
\begin{thm}
Let $(b,c)$ identify a Lucas sequence $u_n$ with $1 \leq b,c \leq 10$ and relatively prime.  Then for all solutions $u_n = y^p$, $p \leq 19$.
\end{thm}

For any prime $p$, given a bound $n \leq B(n,p)$ we develop a sieve to find all $p$th powers.  This method has some similarity to the sieve in \cite{siksek06}; however, unlike their sieve which uses Frey curves, our method relies entirely upon elementary techniques.  Moreover, our method is completely general, applying to any Lucas sequence and any index $n$ (in contrast, \cite{siksek06} relies upon the fact that $n$ is prime and $c = 1$).

Although the sieve is completely general, the bound $B(n,p)$ is sharpest when $c = 1$ and $b^2+4$ is prime.  To demonstrate this method, we prove the following theorem.
...
For $K(S) > B(n,p)$, it is clear that the set of congruences are a complete list globally.  The congruences $n \equiv 0,1 \pmod{K(S)}$ will always be present in $\N(S)$ as they are $p$th powers for any $p$.  In addition, if there exists another $p$th power, this will appear as an added congruence in $\N(S)$ that does not grow with the modulus.  All remaining elements of $\N(S)$ should grow with the modulus $M$.  Clearly, we can rule out these possible congruences if the smallest such $\bar{a} > B(n,p)$.  An implementation of the sieve is available at \url{my_website_to_be_inserted}.


\subsection{Proof of Theorem \ref{cond_examples}}

Using the methods of \ref{condbound}, we find that for the sequences in question, $p\leq 17$.  For a solution $(n,y,p)$ to \eqref{gen_diophan}, with can reduce to odd index $n$ via a generalization of an argument of Robbins \cite{robbins83}.  When $n$ is odd, as in \cite{siksek06}, we convert a solution $(n,y,p)$ to \eqref{gen_diophan} into an integral solution $(X,Y)$ of the Thue equation \begin{equation}
 ...
 ***this is actually still running at the moment, so we'll see in a day or two, but this is almost certainly true.  we have already checked up to $p^{10p}$ **.
