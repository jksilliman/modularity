...
Here, we revisit this strategy with two aims. First, we would like to apply the modular method to find all perfect powers in other linear binary recurrence relations.  We will be particularly interested in natural generalizations of the Fibonacci sequence, known as Lucas sequences.  A Lucas sequence is a nondegenerate recurrence sequence defined by \[ u_n = b u_{n-1} + c u_{n-2}, u_0 = 0, u_1 = 1, \] for b and c nonzero integers.  The Diophantine problem of interest, of course, is to study integer solution $(n,y,p)$, $n > 0, p$ prime, to \begin{equation}\label{the_eqn}u_n = y^p.\end{equation}
