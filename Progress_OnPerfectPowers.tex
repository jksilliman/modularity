...
Assume $p \geq 5$. We verify that the argument of Robbins for the Fibonacci sequence applies more generally.

**TODO: Remove constraint on d**
\begin{prop}
Suppose $b^2 + 4 = d < 100000$, d prime. If $b \neq 1$, there are no solutions $F(2n) = c^p$, $c > 1$, such that $F(m) \neq z^p$ for $m < 2n$.
\end{prop}
\begin{proof}
...
$c^p = F(2n) = F(n)L(n)$, and $(F(n), L(n)) = 1$, so $F(n) = x^p$, contradicting minimality unless $x=1$, in which case $n=2$. Thus $c^p = F(4) = L(2)F(2) = b^2 + 2$. For $d < 100000$, the only solution to this is $(b,c,p) = 5, 27, 3)$. **TODO - Solve equation fully, using Thue equations, perhaps?**
...
At this point, we run a computer search. For $1 \leq d \le 10^5$, we find that none of $F(12), 2^{-1}L(12), \frac{F(9)}{F(3)}$ are perfect powers, with the exception of the Fibonacci sequence itself, where only $F(12)$ is a perfect power.
\end{itemize}
\end{proof}
...
\end{prop}
\begin{proof}
Robbins proof for Fibonacci works for our sequences as well, since d is prime, and we check that $\frac{d^2}{d^2}$ is not a perfect power for $d < 100000$.
